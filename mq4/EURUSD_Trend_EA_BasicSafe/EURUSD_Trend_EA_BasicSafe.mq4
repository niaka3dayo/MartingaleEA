...
// -------------------------------------------------------------------
// --- 下記は必要に応じてオンにする追加フィルター(初期OFF) ---
// -------------------------------------------------------------------

// 1) ボラティリティフィルター & 動的スリッページ & 動的ロットサイズ
extern bool   UseVolatilityFilter       = false;   // ATRで高ボラを判定
extern int    ATRPeriodForVolatility    = 14;      // ATR計算期間
extern double ATRVolatilityThreshold    = 0.0020;  // ATR閾値
extern bool   LimitHighVolatility       = true;    // 高ボラ時にトレード回避
extern bool   UseDynamicSlippage        = false;   // スリッページをATRベースで調整
extern double SlippageATRMultiplier     = 0.2;     // ATRに応じたスリッページ係数
extern bool   UseDynamicPositionSize    = false;   // 高ボラ時にロット縮小
extern double VolatilityReductionFactor = 0.5;     // 縮小率(例:50%)

// 2) ニュースフィルター (デフォルトはOFF)
extern bool   UseNewsFilter    = false;
extern string ImportantNewsTimes = "2025.12.15 15:30"; // 例: ;区切りで複数指定
extern int    PreNewsMinutes   = 30;
extern int    PostNewsMinutes  = 30;

// 3) マルチタイムフレーム(デフォルトOFF)
extern bool UseMultiTimeframeConfirmation = false;
extern int  MTFPeriod = PERIOD_H4;  // H4=240
...
int      g_todayTrades     = 0;
int      g_consecLoss      = 0;
datetime g_todayDate       = 0; // 当日の日付(00:00:00)
